...
PathMonitor.prototype = {
   ...
   _childAdded: function(key, data) {
      ...
      if (profile)
      {
        this.esc.index(this.index, this.type, profile, key)
           .on('data', function(data) {
              console.log('indexed'.green, name);
              console.log('profile'.green, JSON.stringify(profile, null, 4))
           })
      ...
      if (profile)
      {
        this.esc.index(this.index, this.type, profile, key)
           .on('data', function(data) {
              ...
              console.log('profile'.cyan, JSON.stringify(profile, null, 4))
           })
           .on('error', function(err) {
